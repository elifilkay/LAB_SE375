...
public class lab6 {
    ...
    public static void main(String[] args) {
        ...
        String[] fileUrls = {
                "http://homes.ieu.edu.tr/eokur/sample0.txt",
                "http://homes.ieu.edu.tr/eokur/sample1.txt",
                "http://homes.ieu.edu.tr/eokur/sample2.txt",
                "http://homes.ieu.edu.tr/eokur/sample3.txt",
                "http://homes.ieu.edu.tr/eokur/sample4.txt"
        };

        ExecutorService executorService = Executors.newFixedThreadPool(fileUrls.length);

        for (String fileUrl : fileUrls) {
            executorService.submit(() -> processFile(fileUrl, caseChoice, shiftAmount, colorChoice));
        }

        executorService.shutdown();
    }

    private static void processFile(String fileUrl, String caseChoice, int shiftAmount, String colorChoice) {
        ...
        try {
            ...
            System.out.println("Original for " + fileUrl + ":\n" + originalText);

            for (String[] content : characterMap.values()) {
                System.out.println("After Case Change for " + fileUrl + ":\n" + content[1]);
                System.out.println("After Shift for " + fileUrl + ":\n" + content[2]);
                System.out.println("After Color Change for " + fileUrl + ":\n" + content[3] + content[1] + ANSI_RESET);
